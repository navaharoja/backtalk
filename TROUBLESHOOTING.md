...
- **In hands-free or tap-to-listen it keeps hearing things nobody said** ("thank you", "thanks for watching", "you", a row of dots) and they interrupt its reply: that is Whisper hallucinating on silence or room noise — it was trained on video captions and fills a near-empty clip with a stock caption phrase. `ears.transcribe()` drops a transcript that is *entirely* one of those known phrases, and the open-mic voice detector defaults to webrtcvad aggressiveness 3. If it still leaks, raise `vad_aggressiveness` toward 3 (max) and `vad_min_speech_ms` (how much real speech an utterance needs before it counts) in `backtalk.json`, pin `mic_device` so it is not on a noisy far-field input, or just use push-to-talk, where the button brackets your speech and there is no silence to hallucinate over. None of these three keys affect push-to-talk.
